...
class DAPOBatchRewardManager:
    ...
    def __call__(self, data: DataProto, return_dict=False):
        ...
        for i in range(len(data)):
            ...
            # DAPO
            if self.overlong_buffer_cfg.enable: # type: ignore
                overlong_buffer_len = self.overlong_buffer_cfg.len # type: ignore
                expected_len = self.max_resp_len - overlong_buffer_len
                exceed_len = length - expected_len
                overlong_penalty_factor = self.overlong_buffer_cfg.penalty_factor # type: ignore
                overlong_reward = min(-exceed_len / overlong_buffer_len * overlong_penalty_factor, 0)
                reward += overlong_reward
                if self.overlong_buffer_cfg.log: # type: ignore
